    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            match self {
                ...
                RectangularColorSpace::SrgbLinear => {
                    "srgb-linear"
                }
                ...
                RectangularColorSpace::Xyz => {
                    "xyz"
                }
                RectangularColorSpace::XyzD50 => {
                    "xyz-d50"
                }
                RectangularColorSpace::XyzD65 => {
                    "xyz-d65"
                }
            }
        )
...
impl Display for ColorInterpolationMethod {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "in {}",
            match self {
                ColorInterpolationMethod::RectangularColorSpace(rcs) => {
                    rcs.to_string()
                }
                ColorInterpolationMethod::PolarColorSpace(pcs, him) => {
                    format!(
                        "{}{}",
                        pcs,
                        him.map(|v| format!(" {}", v)).unwrap_or_default()
                    )
                }
            }
        )
...
pub enum CssColor {
    ...
    LightDark(Box<CssColor>, Box<CssColor>), // TODO: Relative & Color Function
}
            ...
            CssColor::ColorMix(cip, c1, p1, c2, p2) => {
                write!(
                    ...
                    format!(
                        "{}{}",
                        c1,
                        p1.map(|v| format!(" {}", v)).unwrap_or_default()
                    ),
                    format!(
                        "{}{}",
                        c2,
                        p2.map(|v| format!(" {}", v)).unwrap_or_default()
                    ),
                )
            }
